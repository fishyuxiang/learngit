...
struct dnsdetaillist dnslist[4000000];
...
int collectdnsdetail(char logname[], time_t beginhour, time_t endhour) //提取log文件中时间，ip,线路类型字段，保存于dnsdetaillist结构体数组dnslist中
{
	detaillines = 0;
	FILE *read;
	time_t t;
...
int queryhourdnsdetail(char txtname[], char ip[]) //查询某天24小时某域名各线路访问数据
{
	FILE *read;
	char name[100] = { }, line[512], *str, dn[IPSIZE];
	strcat(name, "/home/yuxiang/test/log/ipdetailhour_");
	strcat(name, txtname);
	strcat(name, ".log\0");
	read = fopen(name, "r");
	if (read == NULL)
...
void querydetail(char ip[], char begintime[], char endtime[]) {
